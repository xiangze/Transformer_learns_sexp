...
\title{Transformers as Functional Dynamics}
...
\begin{abstract}
At first, we explain 
and show the equivalence between attention and Functional dynamics.
\end{abstract}
...
\section{The equivalence between Attention and Functional Dynamics}

%%関数マップとSelf Attentionの等価性、そこから見られるAttentionの文法的機能}

\subsection{先行研究}
Dyck言語を使ったTransformer,Self Attentionの性能に関する理論的研究としては\cite{Hahn_2020},\cite{yao2023selfattentionnetworksprocessbounded}などがある。また\cite{miranda2023transformermodelstypeinference},\cite{flach2025neurallambdacalculusneurosymbolic}ではλ計算、lispのプログラミング支援として多くのS式をTransformer,LLMに学習させている.

前者は主にTransformerのミニマルな性能を理解しようというモチベーションのもとにある研究であり、後者はLisp,S式の自動コード生成、コーディング支援という応用的な目的を志向している。

\cite{clark2019what}などではGPT2tocknizerを用いて自然言語の単語間の関係を行列の値として可視化している。自然言語、プログラミング言語の多様な単語、tokenをデータセットとし、それらの関係をAttention Matrixの要素の値から見出し、わかりやすく可視化している。しかしこれらの研究ではtoken間の相互関係の理解、可視化が主であり、自然言語の節、句に対応するようなtokenのグループ間の関係については余り考察されていない。

\section{実験}
これらの研究を踏まえ、
\subsection{S式の簡約化において見られたAttention Matrix,Headsの特徴}
Attention Matrixを可視化することで
\subsection{精度}
既存のcontextや記憶されたパラメータ内の単語やを鸚鵡返しするのではなく、真に論理的推論を行っているか否かを判断するために限られた単語(数値、関数名)を用いることによってtocknizerが不要な単純な状況を作り、より直接的にAttention Matrixの行列関係関数マップのグラフとして理解することを行う。に対して
実際MLPなしの状況で高階関数を認識し、正しく処理することがでいるかどうかが関数マップ的な構造が効果を持っているのかの指標となる。
...
かつてのCNN,現在のTiVの内部特徴量の可視化にみられるように高階関数、抽象度の高いは層を経るごとに出現頻度を増すのだろうか、といった疑問にも答えることができるかもしれない。
...
そもそもDNNが長らく実用化されなかったのは学習過程において層を経るごとに勾配が消失する問題と過学習の問題のためだとも言われている。Magic Numberの研究はそれとは違ったカオス的性質を持ちうるという意味でDepp MLPが問題をはらんでいると主張している。\cite{kobayashi2024analyzing}では逆にMLPとAttentionの役割分担としてAttentionが混ぜ合わせ、MLPが知識の付加を担っているとしている。
...
関数適用$\cdot$を関手とみなした場合は(圏論としての)極限をとることができるようである。
...
$QK^t\simeq Q(I+k)^t$として摂動展開すれば連続写像として書けるように思われる。しかしSelf Attentionの発展式

$x'=A_x x=QK^T x=W_Q x(W_K x)^T=W_Q x x^T W_K^T$

において$x,W_Q,W_K$を終端文字列として捉えるとこの性質は正規文法では書けず文脈自由文法の範疇になり、既存の力学系で扱う領域を超えるように思われる。

\subsection{批判}
以上の論考で関数マップやTransformer、LLMの新しい性質を導き出せたわけではない。かろうじてAttentionを使ったアーキテクチャの限界を提示しうる可能性があると言えるかもしれない。
\cite{Inoue_2022}でLLMのパラメーター空間の点の学習軌道に対して具体的に示されたように関数マップでLUT(Look Up Table)として表現された関数の軌道の様々な空間での性質(固定点、周期、カオスといった分類は成り立つのか、ヒルベルト空間の元に対しては成り立つが無限に折り畳まれた関数はその元となりうるのか？)とその自然言語、画像処理における意味付けは非常に興味深い研究対象のように思える。

\section{補遺、特異学習理論との関係}
多くの統計モデルやニューラルネットは正規なモデルではなく、その(準安定解)は特異点となっていることが多い。正規な統計モデルの情報量基準(AIC,BIC)を拡張した形で情報量基準(WAIC,WBIC)が定義できるというが特異学習理論(SLT)である\cite{Wata2009}。

最近になってDNNやTransformerの学習過程で選ばれる近似解を説明する性質として注目され、\cite{lau2024LLC}ではloss関数の局所解周辺の学習係数(LLC)を定義、数値的に計算しており、\cite{wang2024differentiationspecializationattentionheads}ではそれを用いてTransfromer,Attention のHeadを機能別に分類している。その中には\cite{funcvec}や\cite{olsson2022incontextlearninginductionheads}で挙げられているInduction HeadやFunction Vectorのようなメタ関数的な機能を持ったものもある。1層のAttentionでそのような機能が実現できることは関数マップの表現能力とも関係しているように予想される。特定の単語に対する反応をrLLC指標で定量化できること、またある素子、HEADを切除または取り替えて出力に現れる違いからHeadが分化していることも示唆されている。
