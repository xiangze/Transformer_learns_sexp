...
\subsection{先行研究}
Dyck言語を使った
λ計算、lispのプログラミング支援として多くのSを母袋板学習指されてい.
\cite{}。
\subsection{S式の簡約化において見られたAttention Matrix,Headsの特徴}

\cite{}などではGPT2tocknizerを用いて自然言語の単語間の関係を行列の値として可視化している。
自然言語、プログラミング言語の多様な単語、tokenを
...
実験はMLPなしの多層AttentionでのS式とその簡約化の組を教師データとしておこなう。
これによって単語,tokenの意味の意味の類似性、階層性ではなく文脈内での意味論的な

かつてのCNN,現在のTiVの内部特徴量の可視化にmられるように高階関数、抽象度の高いは層をヘルドとに出現頻度を増すのだろうか、
